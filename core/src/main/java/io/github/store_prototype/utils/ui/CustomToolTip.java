...
public class CustomToolTip <T extends Actor> extends InputListener {
    static Vector2 tmp = new Vector2();

    private final CustomTooltipManager manager;
    final Container<T> container;
    boolean instant, always, touchIndependent;
    Actor targetActor;

    /** @param contents May be null. */
    public CustomToolTip (@Null T contents) {
        this(contents, CustomTooltipManager.getInstance());
    }

    /** @param contents May be null. */
    public CustomToolTip (@Null T contents, CustomTooltipManager manager) {
    ...
    public CustomTooltipManager getManager () {
        return manager;
    }
    ...
    /** If true, this tooltip is shown even when tooltips are not {@link CustomTooltipManager#enabled}. */
